import { SOL_INFO, txToBase64, TxVersion } from '@gravexio/gravex-sdk'
...
import { getMintSymbol, isSolWSol } from '@/utils/token'
  ...
  () => ({
    ...
    swapTokenAct: async ({ swapResponse, wrapSol, unwrapSol = false, onCloseToast, ...txProps }) => {
      ...
      try {
        ...
        // @ts-ignore
        console.log('simulate tx string:', signedTxs.map(txToBase64) as any)
        ...
        if (e.message !== 'tx failed')
          toastSubject.next({ txError: typeof e === 'string' ? new Error(e) : e, title: 'Trade', description: 'Send transaction failed' })
      } finally {
        txProps.onFinally?.()
      }
      return ''
    ...
    unWrapSolAct: async ({ amount, onSent, onError, ...txProps }): Promise<string | undefined> => {
      ...
      if (builder.allInstructions.length > 12) {
        ...
        multiExecute({
          sequentially: true,
          onTxUpdate: (data) => {
            handleMultiTxRetry(data as unknown as any) // TEW --- POSSIBLE PROBLEM
            handleMultiTxToast({
              toastId,
      ...
      return execute()
        .then(({ txId, signedTx }) => {
          onSent?.()
          // eslint-disable-next-line @typescript-eslint/ban-ts-comment
          // @ts-ignore TEW POSSIBLE PROBLEM
